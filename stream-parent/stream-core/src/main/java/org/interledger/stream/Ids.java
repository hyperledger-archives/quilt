...
public class Ids {
  ...
  @Value.Immutable
  @Wrapped
  @SuppressWarnings( {"checkstyle:TypeName", "UnstableApiUsage"})
  abstract static class _StreamConnectionId extends Wrapper<String> {
    ...
    /**
     * Constructs a {@link StreamConnectionId} that is derived from from the supplied {@code receiverAddress} and {@code
     * streamSharedSecret} by computing an HMAC-SHA-256 of the receiver address using the shared-secret as a key so that
     * the actual shared secret isn't hanging around in memory from call to call. In this way the connection id will
     * also be uniquely scoped to a receiver address plus shared secret combination.
     *
     * @param receiverAddress    The {@link InterledgerAddress} of the receiver of this STREAM payment.
     ...
     */
    public static StreamConnectionId from(
      final InterledgerAddress receiverAddress, final StreamSharedSecret streamSharedSecret
    ) {
      return StreamConnectionId.of(Hashing
        .hmacSha256(Objects.requireNonNull(streamSharedSecret, "streamSharedSecret must not be null").key())
        .hashBytes(
          Objects.requireNonNull(receiverAddress, "receiverAddress must not be null").getValue().getBytes())
        .toString()
      );
