...
/**
 * Unit tests for {@link AesGcmStreamEncryptionService}.
 *
 ...
 */
public class AesGcmStreamEncryptionServiceTest {
  ...
  private AesGcmStreamEncryptionService streamEncryptionService;

  @Before
  public void setUp() {
    this.streamEncryptionService = new AesGcmStreamEncryptionService();
  }
  ...
  public void testDecryptToSameAsRustJsInStandardMode() {
    this.streamEncryptionService = new AesGcmStreamEncryptionService(EncryptionMode.ENCRYPT_STANDARD);
    byte[] decryptedValue = streamEncryptionService.decrypt(SHARED_SECRET, CIPHERTEXT);
    assertThat(decryptedValue).isEqualTo(PLAINTEXT);
  }
  ...
  public void testIvLengthInNonStandardMode() {
    this.streamEncryptionService = new AesGcmStreamEncryptionService(EncryptionMode.ENCRYPT_NON_STANDARD);
    final int actualIvLength = AesGcmStreamEncryptionService.AES_GCM_NONCE_IV_LENGTH;
