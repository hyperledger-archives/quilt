...
public class AimdCongestionController implements CongestionController {
   ...
   *
   * @param startAmount        The packet amount to start a STRAEM at.
   * @param increaseAmount     The amount to increase the {@link #maxInFlight} by whenever a valid fulfillment is
   *                           encountered. This value determines how quickly packet sizes increase as the payment path
   *                           continues to process larger and larger packets.
   * @param decreaseFactor     The amount to lower {@link #maxInFlight} by when the receiver rejects a STREAM packet
   *                           using a T04 or non-F08 error code (F08 rejections will contain information that can be
   *                           used to reduce
   * @param streamCodecContext
   */
  public AimdCongestionController(
   ...
   *
   * @return The current max packet amount for packets in this stream.
   */
  @Override
   ...
   * </p>
   *
   * @return An {@link UnsignedLong} that represents the new value of {@link #maxPacketAmount}.
   */
  @VisibleForTesting
