...
public interface StreamPayer {
  ...
  class Default implements StreamPayer {
    ...
    public Default(final Link<? extends LinkSettings> link, final ExchangeRateProvider oracleExchangeRateProvider) {
...
//    }
    ...
    Default(
      ...
      this.streamEncryptionUtils = StreamEncryptionUtils.sharedStreamEncryptionUtils();
    }
...
//    }
    ...
    private void validateAllocationSchemes(
      ...
      if (sourceAccountDetails.interledgerAddress().getAllocationScheme() !=
        streamConnectionDetails.destinationAddress().getAllocationScheme()
      ) {
        throw new StreamPayerException(
          String.format(
            "Quote failed: incompatible sender/receiver address schemes. sourceAccountDetails=%s, "
              + "streamConnectionDetails=%s", sourceAccountDetails, streamConnectionDetails
          ),
