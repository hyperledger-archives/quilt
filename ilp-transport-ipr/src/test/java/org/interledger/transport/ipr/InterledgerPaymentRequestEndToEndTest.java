...
public class InterledgerPaymentRequestEndToEndTest {
  ...
  public final void test() {
    ...
    Assert.assertTrue("Secret header missing", decryptedPskMessage.getPrivateHeaders("Secret")
        .size() > 0);

    Fulfillment fulfillment = receiverContext.generateFulfillment(decodedPayment);
    Assert.assertTrue("Fulfillment is not valid",
        fulfillment.verify(decodedCondition, new byte[]{}));
  }
